...
func main() {
	...
	for _, item := range records {
		...
		res2 := reg2.ReplaceAllString(temp3, "")
		...
		//remove Apostrophe, spotify API does not like it
		res4 = strings.Replace(res4, "'", "", -1)

		fin_artist := res4

		//remove everything inside () or []
		reg10 := regexp.MustCompile(`[\(\[].*?[\)\]]`)
		res10 := reg10.ReplaceAllString(item[2], "")
		//remove bad words thant contains ***
		reg11 := regexp.MustCompile(`[^ ]*\*\*\*[^ ]*`)
		res11 := reg11.ReplaceAllString(res10, "")
		//remove bad words thant contains **
		reg12 := regexp.MustCompile(`[^ ]*\*\*[^ ]*`)
		res12 := reg12.ReplaceAllString(res11, "")

		res12 = strings.Replace(res12, "'", "", -1)
		fin_track := res12
		...
		results, err := client.Search(context.Background(), q, spotify.SearchTypeTrack, spotify.Limit(1))
